...
def test_eval_if_in_exec_array():
    expect = Element(etype=Etype.NUMBER, value=4)

    evaluator = Evaluator()
    evaluator.eval(to_elems(to_char_gen("{1 {4} if} exec")))
    actual = evaluator.stack.pop()
...
def test_eval_repeat_in_exec_array():
    expect = Element(etype=Etype.NUMBER, value=3)

    evaluator = Evaluator()
    evaluator.eval(to_elems(to_char_gen("{1 2 {1 add} repeat} exec")))
    actual = evaluator.stack.pop()
...
def test_eval_while():
    ...
    evaluator.eval(
        to_elems(
            to_char_gen("5 dup {dup 1 gt} {1 sub exch 1 index mul exch} while pop")
        )
    )
...
def test_eval_exec_arr_nested_action2():

    expect = Element(etype=Etype.NUMBER, value=7)

    evaluator = Evaluator()
    evaluator.eval(to_elems(to_char_gen("{{{1 {3} if} exec} exec 1 {{1}} if  exec add 3 {1 add} repeat} exec")))
    actual = evaluator.stack.pop()
